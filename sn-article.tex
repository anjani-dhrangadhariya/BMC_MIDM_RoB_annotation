...
LLMs (large language models) have immensely capable but necessitate evaluation in RoB assessment.
Currently, no RoB annotation guidelines or annotated corpus exist for LLM evaluation.
% 
\textbf{Methods:} Taking the revised Cochrane Risk of Bias of randomized controlled trials, we develop a set of visual instructional placards that could be used as annotation guidelines to annotate RoB spans in RCTs.
...
The annotated corpus was used to evaluate ChatGPT using a straightforward accuracy evaluation framework.
% 
\textbf{Results:} We present a corpus of 60 full-text RCTs with fine-grained span annotations comprising 100,000 tokens belonging to 22 RoB classes.
...
Utilizing visual placards for bias assessment and annotation enhances inter-annotator agreement (IAA) compared to cases where visual placards are absent.
While LLMs demonstrate potency, their accuracy falters with more subjective bias assessment categories.
...
However, with the advent of LLMs that encode vast amounts of knowledge, full supervision with large amounts of annotated data is not required.
However, the bar for clinical applications is high and it is imperative to evaluate these models for the more challenging clinical tasks like bias extraction~\cite{singhal2023large}.
...
\begin{quote}
\itshape Example prompt: Question 4.3 Were outcome assessors aware of the intervention received by study participants? Extract sentences that you write your answer based on.
\end{quote}
...
%In the analyzing step, ChatPDF creates a semantic index over all paragraphs of the PDF.
...
When the text extracted by LLMs as evidence for answering the signaling question fuzzy matches the text selected by humans, it is considered a correct answer.
In several instances, there was no information found by the expert reviewer from the RCT to answer a question. 
...
It's important to highlight that the evaluation framework is designed solely to measure the accuracy of the answers generated by ChatPDF.
Unlike the manual task, the framework does not require ChatPDF to provide labels.
This evaluation was conducted for 10 out of the 60 RCTs and each document was evaluated using all the 22 signalling questions.
