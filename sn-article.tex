...
With a substantial background in both physiotherapy, advanced statistical methods and experience writing SRs, both experts possesses a deep understanding of the complexities involved in assessing bias risk in RCTs.
...
Two additional researchers with expertise in natural language processing (NLP) were involved, one being a computational linguistics professor and the other a PhD student in computer science.
...
\label{sec:results}
%
This section outlines the visual placards developed, the annotated dataset, presents the inter-annotator agreement findings, and the outcomes of the LLM evaluation.
%
%
%
...
Detailed annotation guidelines and the developed visual placards are available in the Supplementary material, with an example placard instructing to annotate for SQ 3.1 shown in the Figure~\ref{fig:placard}.
The SQ 3.1 ``Were data for this outcome available for all, or nearly all participants randomized?'' assesses the completeness of outcome data in a study.
The question aims to determine if data for the specific outcome of interest were collected and available for analysis for a high proportion of the participants who were initially randomized.
...
Following the above explanation and looking at the placard, the first diamond instructs the annotators to check the Results section (first priority) and the flowchart (second priority) to identify outcome data at the specific time point of interest.
If the annotators find that the outcomes data were available for all or nearly all (at least 95\% of the total) participants, they mark the text descriptions mentioning it and label it with ``3.1 Yes Good''.
If the outcomes data were available for less than 95\% of participants randomized, then they are asked to mark this information and label it with ``3.1 No Bad''.
If no information regarding the availability of outcome data at the specified time point is found, it is considered the question as having ``No Information''.
The placard via the yellow arrow on the first diamond hints that this information is likely found in the Results section of the RCT under consideration, but if not found there, then mark the information in results table. 
If the information is to be found nowhere except in a flowchart, the annotators are instructed to mark the caption of the flowchart.
However, flowchart annotation is used as a last resort and is marked with a red exclamation.
%
%
%
...
The F1-measure scores before and after the guideline improvement were calculated on a different set of documents.
The average overall F1-measure agreement across the corpus exhibited a modest 10.87\% agreement before the introduction of visual placards.
...
We present the Cohen's Kappa ($\kappa$) scores and raw agreement in Table~\ref{tab:IAA_response} which measure agreement at the SQ response judgment level~\cite{mchugh2012interrater}.
...
\begin{table}[]
    ...
    \midrule[1.0pt]
        \multicolumn{4}{c}{Domain 1: Biases arising from the randomization process} \\
        \hline
        1\_1 & 0.8333 & 88.90\% & 22.22\% \\
        1\_2 & 0.5 & 66.70\% & 33.33\% \\
        1\_3 & 0.5556 & 77.80\% & 11.11\% \\
        \hline 
        \multicolumn{4}{c}{Domain 2: Biases due to deviations from intended interventions} \\
        \hline
        2\_1 & 0.7037 & 77.80\% & 5.56\% \\
        2\_2 & 0.5 & 66.70\% & 38.89\% \\
        2\_3 & 0.5 & 66.70\% & 77.78\% \\
        2\_4 & 0.5556 & 77.80\% & 88.89\% \\
        2\_5 & 0.6667 & 77.80\% & 83.33\% \\
        2\_6 & 1 & 100.00\% & 55.56\% \\
        2\_7 & 0.5556 & 77.80\% & 77.78\% \\
        \hline
        \multicolumn{4}{c}{Domain 3: Bias due to missing outcome data} \\
        \hline
        3\_1 & 0.8333 & 88.90\% & 11.11\% \\
        3\_2 & 1 & 100.00\% & 100.00\% \\
        3\_3 & 0 & 33.30\% & 55.56\% \\
        3\_4 & 0.3333 & 55.60\% & 33.33\% \\
        \hline
        \multicolumn{4}{c}{Domain 4: Bias in the measurement of the outcome} \\
        \hline
        4\_1 & 0.5556 & 77.80\% & 11.11\% \\
        4\_2 & -0.5556 & 22.20\% & 38.89\% \\
        4\_3 & 0.6667 & 77.80\% & 27.78\% \\
        4\_4 & 0.1667 & 44.40\% & 22.22\% \\
        4\_5 & 0 & 33.30\% & 66.67\% \\
        \hline
        \multicolumn{4}{c}{Domain 5: Bias in the selection of the reported result} \\
        \hline
        5\_1 & -0.5556 & 22.20\% & 61.11\% \\
        5\_2 & 0.5 & 66.70\% & 22.22\% \\
        5\_3 & 0.3333 & 55.60\% & 72.22\% \\
    \bottomrule[1.0pt]
    \end{tabular}
\end{table}
...
For the first risk domain, ChatGPT has high $P_{O} (Extraction)$ and $P_{O} (Response)$ agreements (66.6\% and 55.3\% IAA respectively) with experts with none of these agreements coming from ``No Information'' responses. % Shows good availability of information for judging bias
For individual SQs in domain 1, $P_{O} (Extraction)$ was greater than $P_{O} (Response)$.
The observed agreements for the the second domain are even higher 64.28\% and 60\% respectively, but 40\% of these agreements emanate from ``No Information'' responses. % Shows a low availability of information to answer a question
Domain 3 shows a moderate observed agreement of 0.475 for both $P_{O} (Response)$ and $P_{O} (Extraction)$.
\textcolor{blue}{The results for risk domain 4 and 5 remain.}
...
%
%
The visual placards addressed various facets of RoB 2 subjectivity and the actual process of annotation.
A common reason for low IAA was when annotators correctly address a bias SQ but annotate evidence from different parts of the full text leading to no agreement. 
...
The placards also tackle subjectivity of certain SQs.
As shown in the Figure~\ref{fig:placard}, the SQ asks to 
...
The evaluation for third domain of bias brought reinforced subjectivity of bias assessment. 
An lenient assessor will judge a risk of bias as low in comparison to a stringent assessor judging a bias risk as high.
...
In other words, they need to assess if the fact that data on falls is missing can be attributed to the actual occurrence of falls.
...
%Another limitation of the study is calculating Cohen's kappa as agreement measure for SQ response option judgment where a percentage of both agreement and disagreement scores came from the ``No information'' judgment meaning from not annotating the evidence. %TODO: Also discuss the Cohen's kappa paradox
%
%
%
\section{Conclusion}
