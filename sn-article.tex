...
Our annotation guidelines and annotations were adapted for benchmarking classical machine learning and not large language models.
So even though we were annotating PDFs, we had to restrict a lot of annotations based on the assumption that PDF will be converted into text via OCR (optical character recognition) losing its structure of tables and figures, which anyway a classical machine learning model could not use without extensive modifications.
This additional restriction of working with PDFs limited our options further.
However, with the advent of LLMs and tools like ChatPDF which allows directly interactions of ChatGPT with PDFs, opened up opportunity of working with PDFs.
We consider it essential to evaluate LLMs instead of forcefully adapting the evaluation to a classical machine learning problem.
...
If the text selected by LLMs is a part of the human selected text, then we attribute it as a correct answer and in this manner we calculate accurary.
...
%TODO: Explain the LLM evaluation framework
It has to be noted that the evaluation framework only aims to measure accuracy of the answer generated by ChatPDF.
%
%
%
\section{Results}
