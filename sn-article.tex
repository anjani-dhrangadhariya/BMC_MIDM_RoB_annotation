...
The histogram of the absolute annotation counts (tokens)~\ref{fig:ann_counts} shows the number of tokens annotated for each RoB signalling questions.
...
For the rest of the SQs, the number of annotated tokens are less than 1000 over the entire corpus. 
There are no token annotations for the bias question 2.4 ``Were these deviations likely to have affected the outcome?'' which depends upon the previous question 2.3.
Table~\ref{table:stats} lists down primary information on the absolute and average annotation length along for each RoB signalling question along with the total number of documents the annotations are identified from.
Annotations from risk domain one are found in almost all the documents while the information pertaining to answering the other risk questions is available only from a handful of the total RCTs annotated in the corpus as also shown in the figure~\ref{fig:rob_information}.
...
While for most questions were information was available, the bias was low as shown by the green bar in comparison to the red bar except for the signalling question number 2.1, 2.2, 3.1, 4.3, 4.4 and 5.2.
%
%
%
\begin{figure}[htb]
