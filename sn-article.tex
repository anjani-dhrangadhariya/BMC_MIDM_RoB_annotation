...
Our annotation guidelines and annotations were adapted for benchmarking supervised machine learning approaches and not large language models.
So even though we were annotating PDFs, we had to restrict a lot of annotations based on the assumption that PDF will be converted into text via OCR (optical character recognition) losing its structure of tables and figures, which anyway a classical machine learning model could not use without extensive modifications~\cite{li2019figure,li2023uttsr}.
...
This is to gauge whether ChatGPT encodes knowledge related to clinical biases.
We will use simple prompt constructs of the structure ``Answer the \textbf{Signalling question} + Action item to extract sentence supporting the answer''. Consider the following example.
...
ChatPDF is expected to extract sentences from RCT PDFs that acts as text evidence to answering the signalling questions.
...
It has to be noted that the evaluation framework only aims to measure accuracy of the answer generated by ChatPDF and does not expect it to give labels as the manual task entails.
